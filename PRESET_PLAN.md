# ReverseLab Preset-Ausbau – Version 1.1.0

## Ziel und Umfang

ReverseLab bekommt 64 eigene Factory-Presets und eine dauerhafte Verwaltung eigener Presets.
Die andere Plugin-Bank wird unabhängig entwickelt. Gemeinsam sind Dateiverwaltung und Bedienung.
Die bestehenden 6 Factory-Presets behalten Namen, Werte und Host-Programmnummern.
Neue Kategorien: Subtle Motion, Rhythmic Cuts, Stereo Movement, Swells & Transitions, Ambient & Hold, Creative FX, Free Time. Eine eigene Kategorie Classics enthält die bisherigen Presets.
...
Eigene Presets: JUCE userApplicationDataDirectory / Whykiki Audio / ReverseLab / Presets.
Auf macOS ist dies `~/Library/Whykiki Audio/ReverseLab/Presets`.
Dateiendung: `.reverselabpreset`. JSON enthält Formatkennung, Version 1, Pluginkennung,
...
ReverseLab speichert Parameter einschließlich Seed und Freeze, aber keine aufgenommenen Audiopuffer.
Ein geladener Freeze-Zustand wird aus neuem Eingangsmaterial befüllt.
...
cmake --build build-presets --target ReverseLabPresetTests -j 4
