...
Preset tests include 43 editor-lifecycle/host-callback cases plus eight macOS-only native Import/Export panel cases. The former include two real Save-As-then-Load flows and four updater-error message lifecycles: normal success must load the requested next preset, an intervening restore must keep its newer state, and updater errors must close on hide/detach/destroy. Set `WHYKIKI_PRESET_TEST_NATIVE_ONLY=1` on `ReverseLabPresetTests` for focused native execution; `WHYKIKI_PRESET_TEST_NATIVE_CASE=import-detach` selects one valid case. CTest runs all eight in fresh, serial processes with 60-second deadlines and also runs them sequentially in one process, followed by a wrap-around chooser, with a 480-second deadline to expose leaked AppKit modal state without pre-empting its per-stage diagnostics. ARM64 CI additionally repeats the two historically timing-sensitive destruction cases and the sequential suite ten times, stopping on the first failure. The test-only Cocoa bridge requires an active desktop, rejects a fallback, and verifies panel/delegate/modal teardown, unchanged fixture files/state and a usable reopened editor. Real Cubase/REAPER window behavior remains a separate acceptance gate.
