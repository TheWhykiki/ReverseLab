...
#if !PRESET_TEST_SUBLAB
    // Exercise a non-default, non-limit size so editor construction/recreation
    // cannot silently replace the processor's persisted editor-size state.
    processor.setLastEditorSize(1030, 690);
    const LifecycleSnapshot beforeEditorCreation(processor);
    auto preparedEditor = std::unique_ptr<juce::AudioProcessorEditor>(processor.createEditor());
    beforeEditorCreation.unchanged(processor);
    LifecycleEditor owner(processor, std::move(preparedEditor));
    beforeEditorCreation.unchanged(processor);
#else
    LifecycleEditor owner(processor);
#endif
