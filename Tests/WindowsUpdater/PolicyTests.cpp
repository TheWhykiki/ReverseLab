...
int main()
{
    ...
    const auto installed = parseVersion("1.1.0");
    const auto newer = parseStableTag("v1.1.1");
    require(installed && newer && isStrictlyNewer(*newer, *installed), "strict stable upgrade");
    require(! parseVersion("01.1.0") && ! parseVersion("1.1") && ! parseVersion("1.1.0.1"),
    ...
    require(! parseStableTag("1.1.1") && ! parseStableTag("v1.1.1-rc1"),
            "stable tags require exact vMAJOR.MINOR.PATCH");
    require(! isStrictlyNewer(*installed, *installed)
                && ! isStrictlyNewer(*parseVersion("1.0.99"), *installed),
            "equal versions and downgrades rejected");

    require(isSafeRepositoryComponent("TheWhykiki") && isSafeRepositoryComponent("ReverseLab"),
    ...
    require(expectedAssetName("ReverseLab", *newer, Architecture::x64)
                == "ReverseLab-1.1.1-Windows-x64.msi",
            "exact x64 asset name");
    require(expectedAssetName("ReverseLab", *newer, Architecture::arm64ec)
                == "ReverseLab-1.1.1-Windows-arm64ec.msi",
            "exact ARM64EC asset name");
    require(expectedAssetUrl("TheWhykiki", "ReverseLab", "ReverseLab", *newer, Architecture::x64)
                == "https://github.com/TheWhykiki/ReverseLab/releases/download/v1.1.1/"
                   "ReverseLab-1.1.1-Windows-x64.msi",
            "exact repository, tag and asset URL");
    require(releasesApiUrl("TheWhykiki", "ReverseLab")
                == "https://api.github.com/repos/TheWhykiki/ReverseLab/releases?per_page=100",
            "exact bounded releases API URL");
    require(releaseByIdApiUrl("TheWhykiki", "ReverseLab", 123456789)
                == "https://api.github.com/repos/TheWhykiki/ReverseLab/releases/123456789",
    ...
    ReleaseGateAuthorizationFields authorization {
        "TheWhykiki", "ReverseLab", "ReverseLab", Architecture::x64,
        *installed, 123456789, "v1.1.1",
    ...
    const std::string expectedAuthorization =
        ...
        "schemaVersion=1\n"
        "repository=TheWhykiki/ReverseLab\n"
        "product=ReverseLab\n"
        "architecture=x64\n"
    ...
    requireMutationChangesAuthorization(mutation);
    mutation = authorization;
    mutation.installedVersion = *parseVersion("1.0.9");
    requireMutationChangesAuthorization(mutation);
    mutation = authorization;
    ++mutation.releaseId;
    requireMutationChangesAuthorization(mutation);
    ...
    require(isCanonicalGuid("401FF4C1-8222-5AB2-987E-AD712164A814")
                && ! isCanonicalGuid("{401FF4C1-8222-5AB2-987E-AD712164A814}"),
            "canonical UpgradeCode format");

    const std::string currentCode = "401FF4C1-8222-5AB2-987E-AD712164A814";
    const std::string otherCode = "28025E3A-AEE3-5516-8BB8-8FCD65D902D6";
    const std::vector<MsiUpgradeRow> upgrades {
        { currentCode, "", "1.1.1", "1033", 0x001, "", "WIX_UPGRADE_DETECTED" },
        { currentCode, "1.1.1", "", "1033", 0x002, "", "WIX_DOWNGRADE_DETECTED" },
