...
The **Updates...** button in ReverseLab's VST3 editor opens a separate native
...
5. The updater verifies the system package receipt, version, every payload file
   ...
   `~/Library/Application Support/Whykiki Audio/Update Backups/ReverseLab/`.
...
`~/Library/Application Support/Whykiki Audio/Updates/ReverseLab/<transaction-id>/`.
...
through the plugin's Updates button, or open that saved `ReverseLabUpdater.app`
...
- Repository: `TheWhykiki/ReverseLab`; HTTPS GitHub Releases API, no embedded token.
...
- Exactly one asset named `ReverseLab-MAJOR.MINOR.PATCH-macOS-universal.pkg`,
...
- One component package, identifier `audio.whykiki.reverselab.pkg`,
  exact release version, install-location `/`, non-relocatable, no installer
  scripts and no payload outside `Library/Audio/Plug-Ins/VST3/ReverseLab.vst3`.
...
CMake builds and embeds `Contents/Helpers/ReverseLabUpdater.app` before the final
...
The current source version is 1.1.0; bump `project(... VERSION ...)` for a new
...
`ReverseLab-MAJOR.MINOR.PATCH-macOS-universal.pkg`; the companion bundle archive
is `ReverseLab-MAJOR.MINOR.PATCH-macOS-universal-VST3.zip`. It writes a new
...
`REVERSELAB_APPLICATION_IDENTITY` and `REVERSELAB_INSTALLER_IDENTITY`, and an existing
keychain notary profile via `REVERSELAB_NOTARY_PROFILE`. Set
`REVERSELAB_NOTARY_KEYCHAIN` to the absolute, existing regular-file path of the
...
CTest includes `ReverseLabUpdaterPolicy` and the existing release-pipeline contract
